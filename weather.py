...
def initialize_weather(b, logger):
    swap.Clear()
    ...
    weatherImage = display.Image.new('RGB', (width, height))
    weatherDraw  = display.ImageDraw.Draw(weatherImage)

    weatherDraw.text((2, 0), 'IT IS ' + weather + ' FUCKING DEGREES' , font=display.font, fill=red)
    weatherDraw.text((2, 16), '& ' + conditions + ' OUTSIDE', font=display.font, fill=green)
