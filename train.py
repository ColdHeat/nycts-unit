...
def load_train_screen():
    ...
    display.draw.rectangle((0, 0, width, height), fill=black)
    display.draw.text((lOffset, 0 + topOffset), lLabel, font=font, fill=orange)
    display.draw.text((fontXoffset + dirOffset, 0 + topOffset), dirLabel, font=font, fill=green)
    display.draw.text((time1Offset, 0 + topOffset), min1, font=font, fill=orange)
    display.draw.text((minOffset, 0 + topOffset), minLabel, font=font, fill=green)

    display.draw.text((lOffset, 16), lLabel, font=font, fill=orange)
    display.draw.text((fontXoffset + dirOffset, 16), dirLabel, font=font, fill=green)
    display.draw.text((time2Offset, 16), min2, font=font, fill=orange)
    display.draw.text((minOffset, 16), minLabel, font=font, fill=green)

    display.draw.point((width - 12, 7), fill=black)
    display.draw.point((width - 12, 20), fill=black)
